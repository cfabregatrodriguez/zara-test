	...
	<section v-if="!isMobile" ref="container" class="scroll-wrapper">
		<div ref="carousel" class="horizontal-carousel">
			<div class="scroll-carousel__slide" v-for="(img, index) in images" :key="index" @click="scrollToImage(index)" :style="{ '--image-height': dynamicHeight }">
				...
				<p v-if="img.text" class="scroll-carousel__text" v-html="img.text" />
			</div>
		</div>
	</section>

	<swiper v-else :slides-per-view="'auto'" :slides-per-group="1" :space-between="16" :loop="true" :centeredSlides="false" :freeMode="false" :pagination="{ clickable: true }" :navigation="true" class="mobile-swiper" :style="{ '--image-height': dynamicHeight }">
		<swiper-slide v-for="(img, index) in images" :key="index" class="scroll-carousel__slide">
			<ResponsiveImage :imageName="img.name" :alt="`Imagen carrusel ${index + 1}`" />
			<p class="scroll-carousel__caption">{{ img.caption }}</p>
		</swiper-slide>
	</swiper>
